...
public:
    RobotNewsStationNode() : Node("robot_news_station"), robot_name_("R2D2") // MODIFY NAME
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Robot News Station from CPP has started");
    }
...
private:
    void publishNews()
    {
        ...
        msg.data = std::string("comment from ") + robot_name_ + std::string("from the CPP robot news station");
